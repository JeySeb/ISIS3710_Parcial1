...
export class TiendaService {
   ...
   async findAll(): Promise<TiendaEntity[]> {
       return await this.tiendaRepository.find({ relations: ["aerolineas"] });
   }

   async findOne(id: string): Promise<TiendaEntity> {
       const tienda: TiendaEntity = await this.tiendaRepository.findOne({where: {id}, relations: ["aerolineas"] } );
       if (!tienda)
   ...
   async create(tienda: TiendaEntity): Promise<TiendaEntity> {
        if (tienda.telefono.length !== 10 )
           throw new BusinessLogicException("The tienda with the given id should have a telefono with at least 10 characters", BusinessError.NOT_FOUND);
